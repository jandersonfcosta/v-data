...
function vData(data) {
    ...
    function bind(elements, _data, callback) {
        ...
        for (var i = 0; i < elements.length; i++) {
            ...
            if (element.attributes["v-repeat"]) continue;

            // v-data="value"
            var vdata = element.attributes["v-data"];

            if (vdata) {
                var value = getData(vdata.value, _data);
            ...
            if (vshow) {
                ...
                if (getData(vshow.value, _data)) element.style.display = "";
            }
            ...
            if (vdisabled) {
                enable(element);
                if (getData(vdisabled.value, _data)) disable(element);
            }
            ...
            if (venabled) {
                disable(element);
                if (getData(venabled.value, _data)) enable(element);
            }
    ...
    function repeatBind(templates, callback) {
        ...
        for (var t = 0; t < templates.length; t++) {
            ...
            for (var d in _data) {
                ...
                if (childs.length) bind(element.querySelectorAll("[v-data]"), item);
